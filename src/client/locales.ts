...
export const en = {
  ...
  modelsClaudeContextDefault: "Known default {default} tokens",
  modelsClaudeContextHint: "Leave blank to use the known model window (Claude 5: 1,000,000 tokens). Larger values adjust DSH local history and compaction only; they do not expand Anthropic API capacity and may exceed the actual model limit.",
...
export const zh = {
  ...
  modelsClaudeContextDefault: "已知默认值 {default} token",
  modelsClaudeContextHint: "留空则使用已知模型窗口（Claude 5：1,000,000 token）。更大的数值只调整 DSH 本地历史和压缩时机，不会增加 Anthropic API 容量，且可能超过模型实际限制。",
